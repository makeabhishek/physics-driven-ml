...
if __name__ == "__main__":
    ...
    # -- Set PDE inputs (mesh, function space, boundary conditions, RHS, ...) -- #
   ...
   # Source term
    ...
    def solve_pde(vel, u_obs, f, V, bcs):
        T = 1. ; dt = 0.01; t = 0;
        """Solve Wave Equation"""
        u = Function(V, name=“u")
        v = Function(V, name=“v")
                     while t<=T:
                     v-= dt/2*u
                     F = v * u * dx + dt * inner(grad(v), vel**2 * grad(v)) * dx
                     if t<=10*dt:
                          F+= inner(f,v)*dx
                          # Solve PDE (using LU factorisation)
                          # solve(F == 0, u, bcs=bcs, solver_parameters={'ksp_type': 'preonly', 'pc_type': 'lu'}) # heat equation
                          # solve(u * v * dx == solve_pde, p, solver_parameters={'ksp_type': 'gmres’})
                          solve(F == 0, u, bcs=bcs, solver_parameters=={'ksp_type': 'preonly', 'pc_type': 'lu’}))
                          v -= dt / 2 * p
                          t += dt
        return assemble_L2_error(v, u_obs)
